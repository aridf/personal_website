...
```{r}
flights <- read_csv("../../static/map/trips.csv")
head(flights)
```
...
if(file.exists("../../static/map/geos.csv")) {
  geos <- read_csv("../../static/map/geos.csv")
  ...
  if(length(new) > 0) {
    ...
    write_csv(geos, "../../static/map/geos.csv")
  }
} else {
  ...
  geos <- new %>%
    ...
    mutate(airport = new) %>%
    select(airport, lon, lat)
  write_csv(geos, "../../static/map/geos.csv") #write dataset
}
head(geos)
```
