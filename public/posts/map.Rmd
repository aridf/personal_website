...
```{r}
flights <- read_csv("res/map/trips.csv")
head(flights)
```
...
if(file.exists("res/map/geos.csv")) {
  geos <- read_csv("res/map/geos.csv")
  ...
  if(length(new) > 0) {
    ...
    write_csv(geos, "res/map/geos.csv")
  }
} else {
  ...
  geos <- new %>%
    ...
    mutate(airport = new) %>%
    select(airport, lon, lat)
  write_csv(geos, "res/map/geos.csv") #write dataset
}
head(geos)
```
