...
```{r}
flights <- read_csv(here("static", "map", "trips.csv"))
head(flights)
```
...
if(file.exists(here("static", "map", "geos.csv"))) {
  geos <- read_csv(here("static", "map", "geos.csv"))
  ...
  if(length(new) > 0) {
    ...
    write_csv(geos, here("static", "map", "geos.csv"))
  }
} else {
  ...
  geos <- new %>%
    ...
    mutate(airport = new) %>%
    select(airport, lon, lat)
  write_csv(geos, here("static", "map", "geos.csv")) #write dataset
}
head(geos)
```
...
ggplot() + worldmap + 
    ...
    geom_text_repel(data = geos, color = "#eee8d5", aes(x = lon, y = lat, label = airport), 
